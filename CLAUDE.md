...
**やったこと（スライド2本の全数チェックと修正）:**

このセッションはコードではなく **pptx の校正**。`アルゴリズム基礎論 2026X.pptx`（JS版・339枚）と
`アルゴリズム基礎論 2026W.pptx`（C++版・275枚）を機械的に総点検した。**非表示スライドは対象外**。

1. **JS版 2026X（表示290枚）— 検出はこちら、修正はユーザー**
   - **貼ると動かないコード5枚**: 172(バブル)・181(選択) は `for` の `{` が無いのに `}` が余る／
     156 は `function fact(n)` に `{` が無く `function f = 1;`（`int `→`function ` の一括置換跡）／
     293 は `for (const v of s)` の中で `output(s[k])`（`k` 未定義）／158 は `bsearch(dst first, …)` のカンマ欠落
   - **旧仕様の残り**: 302 の `await input(`、203・303 の `parseInt(…, 10)`
   - **数値**: 230 の 2²⁰・2³⁰ 行が一律10倍小さい／130・136 の手間の式（後述）
   - **その他**: 198 の `i < N-1`、189/196/198/288 の未使用 `temp`、288 の未使用 `rand()`、
     161 の `BSeratch2.js`、30 `Node,js`、107 `io,js`、38 `retrun`、82 `ture`、48「をの中から」、
     34「10の-4乗」、15/159/267/278 の `int`・`void`、298 の `!=`、50 の余分な吹き出し、
     331〜339 の旧予備スライドが表示のまま
   - ユーザーが全件修正。132 は `if`/`else` の字下げを直し、`let` は**あえて付けない**
     （ループ後も `i` を使う＝一時変数でないことを示すため。134 の Search3.js が `let i = 0;` を
     for の前に置いているのと対になる）
2. **C++版 2026W（表示241枚）— こちらで21箇所を修正**（バックアップ `…2026W.修正前バックアップ.pptx`）
   - **壊れていたコード**: 82 の `while (first <= last)` に `{` 無し／92 の `long fact(int n)` に `{` 無し／
     **190・192 の `int N = s.Length;`（C#/Javaの書き方。C++に無い）** → 186 と同じ
     `void heapSort(int s[], const int N)` にして該当行を削除／248 の `"Pattern not matched!n"`（`∖` 欠落）／
     232 の `Int i = 0;`／94 の全角カンマ `int dst，int first`／26 の `''∖n"` `''∖n''`（`"` が `''`）
   - **数値**: 166 の 2²⁰・2³⁰（JS版230と同じ誤り）／19 の「10の-4乗」
   - **表記**: 241 の1行だけ生の `\n` → `∖`(U+2216)／134 の `i<N-1`／97 のタイトル「プログラム1」→2／
     153 の `For(`／32「をの中から」／37 の `ユーザ名$` → `%`
   - **ユーザー指示で追加対応**: 予備スライド267〜275を非表示に（非表示25→34枚）／
     `srand(time(NULL))` を `shuffle()` から `main()` へ（224・225枚目と `C++/BogoSort1.cpp`）
   - ユーザー側で 82 のデバッグ出力行 `cout << "First = "…` と注記 `// 6T` を削除、
     224・225 の吹き出し整理とインデント、232 に「テープ1本でソートできる!」を追加

**技術的に確定したこと（再調査不要）:**

- **pptx の機械チェックはこの4点で足りる**（スクリプトはスクラッチパッドなので消えるが手順は再現可能）
  1. `ppt/slides/*.xml` の `<a:t>` と `<a:br>` を拾って本文を復元する。**`<a:br>` は自己終了ではなく
     `<a:br><a:rPr …/></a:br>` なので `<a:br/>` だけ見ると改行を全部落とす**（一度これで嵌まった）
  2. コードらしい図形ごとに `{}` `()` `[]` の対応を数える。負（閉じ過ぎ）が本物のバグ、
     正（開き過ぎ）は Part1/Part2 に分割した継続スライドで正常
  3. `Node/` `C++/` の実ソースから同名関数を取り出し、空白とコメントを潰して突き合わせる。
     スライドはデバッグ出力を省くので不一致自体は正常、差分の中身を見る
  4. 旧仕様・誤字の grep（`await` `parseInt(…, 10)` `retrun` `ture` `int ` `void` `cout` など）
- **非表示スライドは `<p:sld … show="0"`**。付けるだけで隠せる
- **手間の式は C++版が正、JS版が壊れていた**。C++版 64/66 は `s[N] = d;`(3T) と `if (i == N)`(4T) を
  きちんと数えて 8N+26。JS版 128/130 は `s[N] = d;`(4T) を落とし、しかも末尾が「見つかった側」の
  経路だった。直して **8N+29**（→140 は「N=3以上」、152 は 109/829/8029 に連動）
  - JS版136 も `+5`（最後の while 判定）を書きながら結果が 13N+26 → **13N+31** に
  - C++版86 は 82 からデバッグ行が消えたことで注記20個とコード20行が1対1になり、
    `0+0+0+2+3+2+3+n*(5+6+6+8+2+4)+5+5+1 = 31n+21` で整合（88 の 145/238/331 も一致）
- **2²⁰・2³⁰ の桁は両版とも10倍小さかった**。2²⁰≈約100万、2²⁰×20≈約2000万、2²⁴≈約1700万、
  2³⁰≈約10億、2³⁰×30≈約320億、2³⁶≈約690億。2¹⁰ の行だけは元から正しい
- **`srand(time(NULL))` を `shuffle()` の中に置くと実害がある**。同じ秒の間は毎回同じ並びが出る。
  実測（9要素）: 修正前は5秒で 3,583,650 回シャッフルしても終わらず、修正後は **151,548 回で終了**。
  `main()` に1回だけ置くのが正しい（BigSort1 と同じ形）
  - `<cstdlib>` `<ctime>` を書かなくても手元の `g++ -Wall` は通る（`<iostream>` 経由）。
    225枚目に include を足すと枠（4.5インチ・16〜18pt で16行）からはみ出すので入れていない
- **pptx の書き換えは「展開 → XML を直接編集 → `zip` で該当エントリだけ更新」で安全にできる**
  - 行内の文字は N 番目の `<a:t>` を位置指定で置換する（同じ文字列が他にもあるため）
  - **行の削除は要注意**。C++版は1行＝1つの `<a:p>` のこともあれば、`<a:br>` で区切られた
    同一 `<a:p>` 内の複数行のこともある。190/192 は後者で、`<a:p>` ごと消して隣の行まで巻き込んだ。
    **直前の `<a:br>` から対象の run の `</a:r>` までを消す**のが正しい
  - 検証は「変更前後の本文テキストを全文 diff して、意図した行以外が動いていないこと」＋
    「全 XML が整形式」＋「zip の健全性」の3つ

**次回への注意:**

- **247枚目（C++版）の `i <= s.size()-p.size()` は直さない方針**。`size()` が符号なしなので
  パターンが文書より長いと暴走するが、「基本的に起きない」とユーザー判断。
  `C++/BruteForceMatching.cpp` も同じ
- **`C++/BogoSort1.cpp` の変更は未コミットのまま残してある**（`srand` を `main()` へ移動）。
  ユーザーが「大元を修正した」とのことで、**コミットしない**指示
- バックアップ `アルゴリズム基礎論 2026W.修正前バックアップ.pptx` と
  `アルゴリズム基礎論 2026X.差し替え前バックアップ.pptx` は Dropbox 側に置いたまま（git 管理外）
- スライド（pptx）まわりの積み残しは前回から変わらず: Google スライドへの取り込み、
  「文言差34枚」を差し替え対象に含めるかの判断
